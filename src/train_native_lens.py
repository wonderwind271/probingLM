...
def resume_or_initialize_backbone(tokenizer, tokenized_dataset, dataset, probing_layer:list):
    ...
    if os.path.exists(CHECKPOINT_DIR): 
        latest_checkpoint = sorted(os.listdir(CHECKPOINT_DIR), key=lambda x: int(x.split('_')[-1].replace('.pt', '')))[-1]
        ...
        # wrap up `probe model`
        probe_model = VocabProbingGPT2(base_model, tokenizer, probing_layers=probing_layer, loss_type="ce")
        probe_model.to(device)
...
def main(probe_layer:list):
    ...
    wandb.init(project=PROJ_NAME, name=f'first try', resume='allow')
    ...
    for epoch in range(start_epoch, effective_epochs):
        ...
        for batch_no, batch in enumerate(progress_bar):
            print(f'Batch {batch_no} is running on device {device}...')
            ...
            if global_step % WANDB_LOG_EVERY == 0:
                wandb.log({'batch_total_loss': loss.item(), 
                            'batch_gpt2_loss': outputs.loss_main.item(),
                            'batch_lens_loss': outputs.total_probe_loss.item(),
            ...
            # Save checkpoint
            if global_step % CHECKPOINT_INTERVAL == 0 or global_step in [0, 150, 300]:
                save_checkpoint(probe_model, optimizer, scheduler, epoch, epoch_step, global_step, probe_layer)
