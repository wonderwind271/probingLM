...
# layer_option_3 = [11,12,13,14,15,16]
layer_options = [layer_option_1, layer_option_2]
all_layers = list(range(11))
probing_layers = all_layers
loss_type = 'kl' 
PROBE_CHECKPOINT_DIR = f'/scratch/chaijy_root/chaijy2/shuyuwu/experiments/checkpoints/natural_wiki/base/probing-100000'
...
def probe_checkpoint_path_to_model(path, probing_layers):
    ...
    model = GPT2LMHeadModel(config=GPT2Config(n_layer=18)).to(device)
    model.resize_token_embeddings(len(tokenizer))
    model.eval()
...
CHECKPOINTS_DIR = f'/scratch/chaijy_root/chaijy2/shuyuwu/experiments/checkpoints/natural_wiki/base'
    ...
    for epoch in range(effective_epochs):
        ...
        for batch_no, batch in enumerate(progress_bar):
            ...
            try:
                ...
                torch.nn.utils.clip_grad_norm_(probe_model.parameters(), max_norm=2.5)
